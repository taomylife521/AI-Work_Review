...
test('主窗口可见性应在前端挂载时补偿同步并在重新显示时恢复动画', async () => {
  ...
  assert.match(
    appSource,
    /safeListen\('main-window-visibility', isBoolean,\s*\(payload\) => \{\s*syncMainWindowVisibility\(payload\);\s*\}/
  );
  assert.match(
    mainSource,
...
test('浏览器预览环境缺少 Tauri event metadata 时主应用事件监听不应中断挂载', async () => {
  ...
  assert.match(
    appSource,
    /async function safeListen<T>\([\s\S]*guard: \(payload: unknown\) => payload is T,[\s\S]*handler: \(payload: T\) => void \| Promise<void>/
  );
  assert.match(appSource, /return await listen<unknown>\(eventName/);
  assert.match(appSource, /if \(!guard\(event\.payload\)\)/);
  assert.match(appSource, /return \(\) => \{\};/);
  assert.match(appSource, /safeListen\('main-window-visibility', isBoolean/);
  assert.match(appSource, /await safeListen\(\s*'recording-state-changed'/);
  assert.match(appSource, /await safeListen\(\s*'config-changed'/);
  assert.match(appSource, /await safeListen\(\s*'avatar-open-timeline'/);
  assert.match(appSource, /await safeListen\(\s*'screenshot-taken'/);
});
