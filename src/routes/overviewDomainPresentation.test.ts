...
import * as presentation from './overviewDomainPresentation.ts';
import {
...
test('域名展示层不再提供缩写印章或浏览器图标相关逻辑', async () => {
  assert.equal(Object.prototype.hasOwnProperty.call(presentation, 'getDomainInitials'), false);
  assert.equal(Object.prototype.hasOwnProperty.call(presentation, 'getDomainStampClass'), false);
});
...
test('非空显式来源即使清洗后为空也不应回退浏览器统计', async () => {
  const browserUsage = [
    { browser_name: 'Chrome', domains: [{ domain: 'example.com', duration: 120 }] },
  ];
  ...
  assert.deepEqual(
    collectDomainBrowserSources(
      'example.com',
      browserUsage,
      [{ browser_name: '  ', duration: 120 }],
    ),
    [],
  );
});

test('来源缺少百分比时应保持按来源总时长补齐的既有语义', async () => {
  assert.deepEqual(
    buildDomainSourceTrack([{ browser_name: 'Chrome', duration: 25 }], 100),
    [{ browser_name: 'Chrome', duration: 25, percentage: 100, widthPct: 100 }],
  );
  assert.equal(buildDomainPresentation({ urls: [{ url: 'a' }, { url: 'b' }] }).pageCount, 2);
});
