...
test('缺省参数与空容器应安全降级，非法时长和应用名称应被清洗', async () => {
  assert.deepEqual(buildCategoryCompositionSummary(), {
    category: '',
    duration: 0,
    percentage: 0,
    activeRange: null,
  ...
  assert.deepEqual(buildCategoryCompositionSummary({
    ...
    appBreakdown: [
      ...
      {
        apps: [
          { app_name: '  Code  ', category: 'work', duration: 30 },
          { app_name: '', category: 'work', duration: 50 },
          { app_name: 'Docs', category: 'work', duration: -10 },
        ],
      },
    ],
  }), {
    ...
    percentage: 0,
    activeRange: null,
    primaryApps: [{ appName: 'Code', duration: 30 }],
  });
});
