...
export type AvailableAvatarPresetId = 'original-standard';
export type AvatarAssetMap = Record<string, string>;

export interface AvatarPreviewInputActivity {
...
export interface AvatarPresetOption {
  id: AvailableAvatarPresetId;
  titleKey: string;
  descriptionKey: string;
  previewMotionBeat: number;
  previewInputActivity: AvatarPreviewInputActivity;
}

export interface AvatarMotionBounds {
  x: number;
  y: number;
  width: number;
  height: number;
...
export interface AvatarMouseMotionModel {
  anchorX: number;
  anchorY: number;
  bounds: AvatarMotionBounds;
  bend: number;
}

interface AvatarHotspotBase {
  fill: string;
  stroke: string;
}

export interface AvatarRectHotspot extends AvatarHotspotBase {
  kind: 'rect';
  x: number;
  y: number;
  width: number;
...
export interface AvatarEllipseHotspot extends AvatarHotspotBase {
  kind: 'ellipse';
  cx: number;
  cy: number;
  rx: number;
...
export interface AvatarPolygonHotspot extends AvatarHotspotBase {
  kind: 'polygon';
  points: string;
}

export type AvatarHotspot =
  | AvatarRectHotspot
  | AvatarEllipseHotspot
  | AvatarPolygonHotspot;

export interface AvatarInteractionLayout {
  trackpad?: AvatarHotspot;
  keyboardHotspots?: Record<string, readonly AvatarHotspot[]>;
  mouseHotspots?: Record<string, readonly AvatarHotspot[]>;
}
...
export interface AvatarPresetDefinition {
  ...
  renderMode?: 'standard' | 'source-keyboard';
  keyboardFrames: AvatarAssetMap;
  handFrames: AvatarAssetMap;
  idleHand: string;
  keyboardVisualLayers: AvatarAssetMap | null;
  keyboardVisualClip?: AvatarHotspot;
  leftHandFrames?: AvatarAssetMap;
  rightHandFrames?: AvatarAssetMap;
  idleLeftHand?: string;
  idleRightHand?: string;
  mouseDeviceBase: string;
  mouseDeviceOverlays: AvatarAssetMap;
  mouseVisualLayers: AvatarAssetMap | null;
  mouseVisualClip?: AvatarHotspot;
  mouseMotionModel?: AvatarMouseMotionModel;
  interactionLayout?: AvatarInteractionLayout;
  showKeyboardOverlay: boolean;
  showMouseDevice: boolean;
  showMouseArm: boolean;
  keyboardHotspotsAboveCover?: boolean;
  mouseHotspotsAboveCover?: boolean;
  keyboardOverlayOpacity: number;
  handOpacity: number;
  mouseDeviceOpacity: number;
  mouseOverlayOpacity: number;
  mouseArmOpacity: number;
}

export const AVATAR_PRESET_DEFAULT: AvailableAvatarPresetId = 'original-standard';

function createPngLayerMap(
  layerEntries: Readonly<Record<string, string>>,
): AvatarAssetMap {
...
function aliasLayers(
  baseLayers: Readonly<AvatarAssetMap>,
  aliasGroups: readonly (readonly [string, string])[],
): AvatarAssetMap {
  const nextLayers: AvatarAssetMap = {};
...
function rectHotspot(
  ...
  height: number,
  transform: string,
  fill = HOTSPOT_CYAN_FILL,
  stroke = HOTSPOT_CYAN_STROKE,
): AvatarRectHotspot {
...
const STANDARD_MODEL_LEFT_KEYS = createPngLayerMap(
  import.meta.glob<string>('./assets/bongocat/model-standard/left-keys/*.png', {
    eager: true,
    import: 'default',
  })
);
const GAMEPAD_MODEL_LEFT_KEYS = createPngLayerMap(
  import.meta.glob<string>('./assets/bongocat/model-gamepad/left-keys/*.png', {
    eager: true,
    import: 'default',
  })
);
const GAMEPAD_MODEL_RIGHT_KEYS = createPngLayerMap(
  import.meta.glob<string>('./assets/bongocat/model-gamepad/right-keys/*.png', {
    eager: true,
    import: 'default',
  })
);
...
const AVAILABLE_AVATAR_PRESET_IDS: ReadonlySet<string> = new Set(
  AVATAR_PRESET_OPTIONS.map((option) => option.id)
);

const AVATAR_PRESET_REGISTRY: Record<AvatarPresetId, AvatarPresetDefinition> = {
  'original-standard': {
    ...STANDARD_BASE,
    id: 'original-standard',
    sceneSrc: mouseBg,
...
export function normalizeAvatarPresetId(presetId: string): AvailableAvatarPresetId {
  return AVAILABLE_AVATAR_PRESET_IDS.has(presetId)
    ? presetId as AvailableAvatarPresetId
    : AVATAR_PRESET_DEFAULT;
}

export function getAvatarPresetDefinition(presetId: string): AvatarPresetDefinition {
  return AVATAR_PRESET_REGISTRY[normalizeAvatarPresetId(presetId)];
}

export function getAvatarPresetOption(presetId: string): AvatarPresetOption {
