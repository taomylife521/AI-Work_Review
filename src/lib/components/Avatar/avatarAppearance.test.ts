...
test('旧的手绘外观调色辅助应移除，桌宠外观改由预设资源和状态元信息驱动', () => {
  ...
  const stateMetaSource = readFileSync(new URL('./avatarStateMeta.ts', import.meta.url), 'utf8');
...
test('桌宠预设与状态元信息应迁移到 TypeScript', () => {
  assert.equal(existsSync(new URL('./avatarPresetRegistry.ts', import.meta.url)), true);
  assert.equal(existsSync(new URL('./avatarStateMeta.ts', import.meta.url)), true);
});
