  ...
  <div class="absolute inset-0 z-30 overflow-visible pointer-events-none">
    ...
    >
      ...
      <div class="relative mt-3 shrink-0 space-y-2">
        <button
          type="button"
          class={`inline-flex w-full items-center justify-center rounded-lg px-3 py-2 text-xs font-semibold transition ${copy.primaryClass || 'bg-emerald-500 hover:bg-emerald-600 text-white'}`}
          on:click={onTimeline}
        >
          {copy.openTimeline}
        </button>
        ...
        <div class="flex flex-col gap-2">
          <button
            type="button"
            class="inline-flex min-h-[34px] w-full items-center justify-center gap-1 rounded-lg border border-slate-200 bg-white dark:bg-[#21262d] px-3 py-2 text-xs font-medium text-slate-700 dark:text-[#adbac7] transition hover:border-slate-300 dark:hover:border-[#484f58] hover:bg-slate-50 dark:hover:bg-[#30363d]"
