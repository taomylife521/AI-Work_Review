...
test('桌宠气泡文案应去除首尾空白并拒绝非字符串输入', () => {
  assert.equal(formatBubbleMessage('  正在专注  '), '正在专注');
  assert.equal(formatBubbleMessage('   '), '');
  assert.equal(formatBubbleMessage(null), '');
  assert.equal(formatBubbleMessage(42), '');
});
