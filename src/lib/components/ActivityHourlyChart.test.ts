...
test('按小时活跃度图表应将所选时段直接显示在柱状图内', async () => {
  ...
  assert.match(source, /let selectedHour(?:\s*:\s*number\s*\|\s*null)? = null/);
  assert.match(source, /function selectHour\(hour(?:\s*:\s*number)?\)(?::\s*void)?/);
...
test('按小时活跃度图表文案不应继续维护独立所选时段提示', async () => {
  const source = await readFile(new URL('../i18n/locales/zh-CN.ts', import.meta.url), 'utf8');
...
test('按小时活跃度只保留竖向图并支持所选分类弱化与应用过滤', async () => {
  ...
  assert.match(
    source,
    /export let selectedCategory(?:\s*:\s*string\s*\|\s*null)? = null/,
  );
