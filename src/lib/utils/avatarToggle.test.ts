...
test('关闭桌宠保存失败时应同时恢复桌宠与休息提醒状态', async () => {
  ...
  assert.equal(config.avatar_enabled, true);
  assert.equal(config.break_reminder_enabled, true);
});

test('桌宠开关保存函数同步抛错时应恢复完整旧状态', async () => {
  const config = {
    avatar_enabled: true,
  ...
  await assert.rejects(
    toggleAvatarSetting(config, (nextConfig) => {
      assert.strictEqual(nextConfig, config);
      throw new Error('sync toggle save failed');
    }),
    /sync toggle save failed/
  );
...
test('桌宠缩放保存成功时应以同一配置对象保存裁剪值并返回该值', async () => {
  const config = { avatar_scale: 0.9, marker: 'keep' };
  let savedConfig = null;

  const savedScale = await updateAvatarScaleSetting(config, 2, async (nextConfig) => {
    savedConfig = nextConfig;
    assert.equal(nextConfig.avatar_scale, AVATAR_SCALE_MAX);
  });

  assert.strictEqual(savedConfig, config);
  assert.equal(config.avatar_scale, AVATAR_SCALE_MAX);
  assert.equal(savedScale, AVATAR_SCALE_MAX);
});
...
test('桌宠缩放保存函数同步抛错时应回滚并保留配置对象', async () => {
  const config = { avatar_scale: 0.9, marker: 'keep' };

  await assert.rejects(
    updateAvatarScaleSetting(config, 1.2, (nextConfig) => {
      assert.strictEqual(nextConfig, config);
      throw new Error('sync scale save failed');
    }),
    /sync scale save failed/
  );

  assert.deepEqual(config, { avatar_scale: 0.9, marker: 'keep' });
});
...
test('桌宠透明度保存成功时应以同一配置对象保存裁剪值并返回该值', async () => {
  const config = { avatar_opacity: 0.82, marker: 'keep' };
  let savedConfig = null;

  const savedOpacity = await updateAvatarOpacitySetting(config, 0.1, async (nextConfig) => {
    savedConfig = nextConfig;
    assert.equal(nextConfig.avatar_opacity, AVATAR_OPACITY_MIN);
  });

  assert.strictEqual(savedConfig, config);
  assert.equal(config.avatar_opacity, AVATAR_OPACITY_MIN);
  assert.equal(savedOpacity, AVATAR_OPACITY_MIN);
});
...
test('桌宠透明度保存函数同步抛错时应回滚并保留配置对象', async () => {
  const config = { avatar_opacity: 0.82, marker: 'keep' };

  await assert.rejects(
    updateAvatarOpacitySetting(config, 0.6, (nextConfig) => {
      assert.strictEqual(nextConfig, config);
      throw new Error('sync opacity save failed');
    }),
    /sync opacity save failed/
  );

  assert.deepEqual(config, { avatar_opacity: 0.82, marker: 'keep' });
});
