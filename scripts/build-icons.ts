...
function renderFilter(size: number): string {
  if (size <= 24) {
...
async function buildPngWithFfmpeg(
  size: number,
  output: string,
  source: string
): Promise<void> {
...
async function buildPngWithSips(
  size: number,
  output: string,
  source: string
): Promise<void> {
...
async function buildIcns(tempDir: string): Promise<boolean> {
  if (!hasIconutil) {
...
async function buildNativeIconsWithTauri(tempDir: string): Promise<boolean> {
  if (!hasTauriCli) {
    return false;
  }
...
async function buildIco(tempDir: string): Promise<void> {
