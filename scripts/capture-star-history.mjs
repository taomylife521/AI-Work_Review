...
//
// 用法:  node scripts/capture-star-history.mjs
//        或:  npm run star-history
// 可选:  STAR_HISTORY_REPO=owner/name node scripts/capture-star-history.mjs
//
...
export function buildDailySeries(starDates) {
...
export function niceTicks(maxValue, count = 5) {
  ...
  if (ticks[ticks.length - 1] < maxValue + step * 0.001) {
...
export function renderSvg(series, themeName) {
  const t = THEMES[themeName];
  const plotW = WIDTH - PADDING.left - PADDING.right;
  const plotH = HEIGHT - PADDING.top - PADDING.bottom;

  const firstDay = Date.parse(`${series[0].date}T00:00:00Z`);
  const lastDay = Date.parse(`${series[series.length - 1].date}T00:00:00Z`);
  const spanDays = Math.max(1, Math.round((lastDay - firstDay) / 86_400_000));
  const maxTotal = series[series.length - 1].total;

  const xScale = (dayMs) => PADDING.left + ((dayMs - firstDay) / 86_400_000 / spanDays) * plotW;
  const yTicks = niceTicks(maxTotal, 5);
  // Y 轴上界取顶部刻度而非原始最大值:曲线最高点不贴顶,所有刻度都落在图内。
  const yMax = Math.max(1, yTicks[yTicks.length - 1]);
  const yScale = (total) => PADDING.top + plotH - (total / yMax) * plotH;
